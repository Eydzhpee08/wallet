...
var defaultFavorite = types.Favorite{
	...
	Name:      "IT",
	Amount:    10,
	Category:  types.PaymentCategoryIT,
}
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				accountID: 0,
				amount:    10,
				category:  types.PaymentCategoryFood,
			},
			want:    nil,
			wantErr: true,
		},
...
func TestService_Repeat_success(t *testing.T) {
	...
	payment, err := s.Pay(account.ID, 10, types.PaymentCategoryIT)
	if err != nil {
		t.Errorf("payment => %v", payment)
		return
	}
...
func TestService_FavoritePayment_success(t *testing.T) {
	...
	favorite, err := s.FavoritePayment(payment.ID, ("Some"))
	if err != nil {
...
func TestService_Export(t *testing.T) {
	...
	payment, _ := s.Pay(account1.ID, 10, types.PaymentCategoryIT)
	_, _ = s.FavoritePayment(payment.ID, "IT")

	account2, _ := s.AddAccountWithBalance("9127660306", 11)
	payment2, _ := s.Pay(account2.ID, 10, types.PaymentCategoryIT)
	_, _ = s.FavoritePayment(payment2.ID, "IT")
...
func TestService_HistoryToFiles(t *testing.T) {
	...
	_, _ = s.Pay(account1.ID, 10, types.PaymentCategoryIT)
	_, _ = s.Pay(account1.ID, 100, types.PaymentCategoryIT)
}
